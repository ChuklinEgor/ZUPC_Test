...
Функция ПолучитьДанныеДляПостроенияОтчета(СписокФизЛицОрганизация)
	...
	|	МАКСИМУМ(ВТДоходыДоПолученияСтатусаРезидента.Год) КАК Год,
	|	МАКСИМУМ(ВТДоходыДоПолученияСтатусаРезидента.Месяц) КАК Месяц,
	|	МАКСИМУМ(ВТДоходыДоПолученияСтатусаРезидента.Ставка) КАК Ставка
	|ИЗ
	|	ВТДоходыДоПолученияСтатусаРезидента КАК ВТДоходыДоПолученияСтатусаРезидента
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТДоходыДоПолученияСтатусаРезидента.ГоловнаяОрганизация,
	|	ВТДоходыДоПолученияСтатусаРезидента.ФизическоеЛицо
	|	
	|	";
	
	
	//ЦТЗУП_ОбщегоНазначенияНаСервере.СохранитьЗапрос(Запрос);
...
Функция ПолучитьДанныеДляПостроенияОтчетаОЛД(СписокФизЛицОрганизация)
